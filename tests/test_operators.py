...
@pytest.mark.parametrize(
    "calculation, result", [("5.5 + 10", 15.5), ("2 + 2 + 10", 14), ("4.8 + 5", 9.8)]
)
...
@pytest.mark.parametrize(
    "calculation, result", [("5.5 - 10", None), ("2 + 2 * 10", None)]
)
...
@pytest.mark.parametrize(
    "calculation, result", [("5.5 - 10", -4.5), ("10 - 3 - 2", 5), ("5 - 5", 0)]
)
...
@pytest.mark.parametrize(
    "calculation, result", [("5.5 * 10", 55), ("100 * 2 * 4", 800), ("4.8 * 5", 24)]
)
...
@pytest.mark.parametrize(
    "calculation, result", [("5.5 / 10", 0.55), ("100 / 2 / 10", 5), ("5 / 5", 1)]
)
